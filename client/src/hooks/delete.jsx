...
function uerDeleteApi() {
    //   const { endPoint, params } = apiProps;
    const [data, setData] = useState(null);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const { gHead, addGHead } = useGiraf()
    const headerConfig = {Authorization: gHead.auth_token }
    ...
    const actionRequest = async ({
        ...
        try {
            const { data: res } = await axios.delete(endPoint, { params: params, headers: { ...headerConfig, ...hd } });
            setData(res)
            setLoading(false)
            return res
        } catch (err) {
